...
Consolidated from the original set of legacy services. See **[Service Catalog](/architecture/service-catalog)** for Spring application classes, per-service endpoints, and storage details.
...
<Info>
**Platform infrastructure:** Kafka 3.8.0 (10 topics — see [Kafka topics](/data-apis/kafka-topics)), Redis 7.4 C1 (snapshots, port 6380) + C2 (candles/movers/FNO/alerts, port 6381) — see [Redis keys](/data-apis/redis-keys), ClickHouse (ReplacingMergeTree + 4 materialized view rollups), PostgreSQL 16, Temporal 1.26.2.
</Info>
...
| **Kafka** | 10 topics for inter-service messaging (ticks, candles, instruments, corporate actions, recon, anomalies, alerts, control) | Durable stream; replay as ultimate fallback in three-layer durability model |
...
<Tabs>
  <Tab title="Current (v4.0)">
    - **10 consolidated services** — charts-service (3-in-1), scheduler-service (2-in-1), ops-service (2-in-1), instrument-master absorbs company-fundamentals
